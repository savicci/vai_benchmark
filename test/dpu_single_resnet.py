...
def load_tensorflow_dataset(size = None) -> Tuple[List, List]:
    return resnet_utils.load_tensorflow_dataset(size)


@vai_tracepoint
def preprocess_dataset(images, scale) -> List:
    return resnet_utils.preprocess_dataset(images, scale)


@vai_tracepoint
def postprocess_results(out_vectors, labels):
    resnet_utils.postprocess_results(out_vectors, labels)
...
def app(model, dpu_cores, size):
    # load dataset
    images, labels = load_tensorflow_dataset(size)
...
if __name__ == '__main__':
    ...
    app(args.model, args.dpu_cores, args.size)
