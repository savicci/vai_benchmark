...
def load_model(workspace, prefix) -> tf.keras.models.Model:
    return tf.keras.models.load_model(workspace + '/' + prefix + '/pruned/resnet_model')
...
def app(workspace, calibrations, prefix, batch_size, fast_ft_epochs):
    ...
    # evaluate
    with open(workspace + '/' + prefix + '/quantized/resnet_pruned_evaluate.txt', 'w+') as f:
        with redirect_stdout(f):
            loss, accuracy = quantized_model.evaluate(ds_test, verbose=2)
            print('Loss {}, accuracy {}'.format(loss, accuracy))

    # save init summary
    with open(workspace + '/' + prefix + '/quantized/resnet_init_summary.txt', 'w+') as f:
        with redirect_stdout(f):
            quantized_model.summary()

    # save model
    quantized_model.save(workspace + '/' + prefix + '/quantized/resnet_model.h5')
