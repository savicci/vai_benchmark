...
def load_model(workspace, prefix) -> tf.keras.models.Model:
    return tf.keras.models.load_model(workspace + '/' + prefix + '/trained/resnet_model')
...
def app(epochs, workspace, ratio, prefix):
    ...
    # evaluate
    with open(workspace + '/' + prefix + '/pruned/resnet_pruned_evaluate.txt', 'w+') as f:
    ...
    # save model
    filename = "/tmp/vai_benchmark/data/pruned/resnet_model_sparse"
    ...
    pruned_slim_model.save(workspace + '/' + prefix + '/pruned/resnet_model')

    # save init summary
    with open(workspace + '/' + prefix + '/pruned/resnet_pruned_summary.txt', 'w+') as f:
        with redirect_stdout(f):
